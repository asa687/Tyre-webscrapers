...
def Tyre_database_blackcircles(site, append, width, aspectRatio, rimSize, postcode, isHeader):
    ...
    # the tyre pattern and name are joined together so they must be seperated
    ...
    conn = sqlite3.connect('blacktyres.db') 
    c = conn.cursor()  
    df = pd.read_csv(csv_file, encoding='ISO-8859-1') 
    df.info()
    df.to_sql('blacktyres', conn, if_exists='replace', index=False)
...
append = '/tyres/'
Tyre_database_blackcircles(site, append, '205', '55', '16', postcode, True) 
time.sleep(random.randint(10, 30)) 
